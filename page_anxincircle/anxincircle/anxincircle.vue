						...
						<view class="itemContent">
							...
							<view class="content">
								...
								<view class="buttom">
									...
									<view class="loveIcon">
										<image
											:src="item.momentLikes?item.momentLikes.some(item=>item.id==userInfo.id)?lovedSrc:loveSrc:loveSrc"
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				circleList: [{
						subTitle: '独自旅行',
						title: "独自旅行1",
						avatar: "/static/picture/headImg.png",
						beginDate: "2024-4-2",
						endDate: "2024-4-2",
						photo1: "../../static/icon/homepagebackground.png",
						photo2: "../../static/icon/homepagebackground.png",
						photo3: "../../static/icon/homepagebackground.png",
						photo4: "../../static/icon/homepagebackground.png",
					},
					{
						subTitle: '独自旅行',
						title: "独自旅行1",
		...
		methods: {
			...
			changeLove(idx) {
				if (this.circleList[idx].momentLikes && this.circleList[idx].momentLikes.some(item => item.id == this
						.userInfo.id)) {
					// 如果点了赞的话
			...
			getXinanCircle() {
				...
				this.isLoading = 1;
				this.isRefresh = true;
				getAllXinanCircle(this.pageNum, this.pageSize).then(res => {
		...
		onLoad() {
			...
			uni.$on('addXinanCricle', (res) => {
				...
				this.isLoading = 1;
				this.isRefresh = true;
				updataXinanCircle(res).then(res => {
					console.log("发布心安圈成功", res)
